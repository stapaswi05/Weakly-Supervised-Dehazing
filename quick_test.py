...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        if opt.save_image:
            ...
            util.save_image(dehzImg, os.path.join(curSaveFolder, '%s.png'%(name)))

            #curSaveFolder1 = os.path.join(opt.dataroot, "transmission/")
            refinedT = util.tensor2im(visuals['refine_T_vis'])
            util.save_image(refinedT, os.path.join(curSaveFolder, '%s_T.png'%(name)))
